...
Q.Tool.define(
	...
	function () {
		...
		function _changed() {
			...
			var streamName = "Places/area/" + tool.$location.val() + "/" + tool.$floor.val() + tool.$column.val();
			Q.Streams.get(
				state.communityId,
				streamName,
				function (err, stream, extra) {
					...
					Q.Template.render("Communities/occupants/results", fields, function (err, html) {
						var msg;
						...
						$te.on(
							"click.Communities_occupants",
							".Communities_occupants_setIdentifier",
							tool,
							function () {
						...
						$te.on("click.Communities_occupants", ".Communities_occupants_invite", tool, function () {
							if (!state.currentStream) {
								return;
							}
							Q.Streams.invite(
								state.communityId,
								state.currentStream.fields.name,
								{
