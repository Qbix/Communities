...
Q.exports(function (options, index, column, data) {
	var userId = NFTRemix.profile.userId;
	var $profileColumn = $(".Communities_NFTprofile_column", column);
	if (!$profileColumn.length) {
		return;
	}

	Q.addStylesheet('{{Communities}}/css/columns/NFTprofile.css', {slotName: 'Communities'});

	Q.Text.get('Assets/content', function (err, content) {
		$(".Communities_NFTprofile_logout").on(Q.Pointer.fastclick, function () {
	...
	$profileColumn[0].forEachTool("Assets/NFT/preview", function () {
		this.state.onInvoke.set(function (publisherId, streamName) {
			...
			var options = {
				...
				name: 'NFT',
				url: Q.url("Communities/NFT/" + publisherId + "/" + lastPart),
	...
	$(".Communities_social_icon[data-type=facebook]", column).on(Q.Pointer.fastclick, _socialHandlerProfile);
	$(".Communities_social_icon[data-type=twitter]", column).on(Q.Pointer.fastclick, _socialHandlerProfile);
	$(".Communities_social_icon[data-type=linkedin]", column).on(Q.Pointer.fastclick, _socialHandlerProfile);
	$(".Communities_social_icon[data-type=github]", column).on(Q.Pointer.fastclick, _socialHandlerProfile);
	$(".Communities_social_icon[data-type=instagram]", column).on(Q.Pointer.fastclick, _socialHandlerProfile);
});
