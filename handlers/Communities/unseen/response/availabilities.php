<?php
// if defined $_REQUEST[publisherId] and $_REQUEST[streamName], check if this availability stream suitable for current user.
// Otherwise, return suitable streams details.
function Communities_unseen_response_availabilities($params)
{
	$fields = array(
		'publisherId' => null,
		'streamName' => null,
	...
	if (!empty($r['publisherId']) && !empty($r['streamName'])) {
		// check whether new availability exist in appropriate relations
