...
function Communities_after_Places_geolocation($params)
{
	...
	if ($created) {
		$q = Places_Location::select()->where(array(
			'publisherId' => new Db_Range('A', true, true, 'Z'),
			'streamName like ' => 'Places/location/%'
		));
		$res = Places_Geohash::fetchByDistance($q, 'geohash', Places_Geohash::encode($stream->getAttribute('latitude'), $stream->getAttribute('longitude')), 1);
